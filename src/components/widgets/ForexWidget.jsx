    ...
    return (
        <div className="bg-black/80 border border-white/10 rounded-xl p-4 w-full h-full flex flex-col relative overflow-hidden">
            ...
            <div className="flex justify-between items-end mb-4 z-10">
                <div>
                    <h3 className="text-slate-400 text-sm font-semibold">EUR/USD</h3>
            ...
            <div className="grid grid-cols-3 gap-2 mt-4 z-10">
                {['15m', '1H', '4H'].map(t => (
                    <div key={t} className="text-center text-xs text-slate-500 py-1 bg-white/5 rounded hover:bg-white/10 cursor-pointer transition-colors">{t}</div>
                ))}
            </div>
        </div>
    )
